function Initialize-AzureImageBuilder {
    ...
    try {
        ...
        New-AIBResourceGroup -ResourceGroupName $ResourceGroupName -Location $Location -Force
        #endregion

        #region Create managed identity
        Write-Host "Creating managed identity.." -ForegroundColor Yellow
        $managedIdentity = New-AIBManagedIdentity -ResourceGroupName $imageResourceGroup -IdentityName $IdentityName
        #endregion
